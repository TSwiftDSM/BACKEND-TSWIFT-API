...
class FornecedorProduto {
    ...
    async getPorIdProdutoFornecedor(req: Request, res: Response) {
        try {
            ...
            const fornecedorProduto = await fornecedorProdutoServices.getPorIdProdutoIdFornecedor(parseInt(idProduto), parseInt(idFornecedor))
    ...
    async getPorIdFornecedor(req: Request, res: Response) {
        try {
            const { id } = req.params
            const fornecedorProduto = await fornecedorProdutoServices.getPorIdFornecedor(parseInt(id))
            return res.status(200).json(fornecedorProduto);
        } catch {
            res.send("Erro ao retornar dados");
        }
    ...
    async delete(req: Request, res: Response) {
        try {
            const { idProduto, idFornecedor } = req.params
            await fornecedorProdutoServices.delete(parseInt(idProduto), parseInt(idFornecedor))
