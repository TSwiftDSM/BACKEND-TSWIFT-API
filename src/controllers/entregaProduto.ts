...
class EntregaProduto {
    async getPorId(req: Request, res: Response) {
        try {
            const { id } = req.params
            const entregasProdutos = await entregaProdutosServices.getPorId(parseInt(id));
    ...
    async getPorIdEntrega(req: Request, res: Response) {
        try {
            ...
            const entregasProdutos = await entregaProdutosServices.getPorIdEntrega(parseInt(idEntrega));
            res.status(200).json(entregasProdutos);
        } catch {
            res.send("Erro ao retornar dados");
        }
    ...
    async update(req: Request, res: Response) {
        try {
            const { id } = req.params
            const data = req.body
            const entregaProduto = await entregaProdutosServices.update(data, parseInt(id))
